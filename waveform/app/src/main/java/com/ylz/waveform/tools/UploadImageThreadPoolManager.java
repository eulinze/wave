 ...
 */
public class UploadImageThreadPoolManager {
    /**
     * 单例设计模式（饿汉式）
     *  单例首先私有化构造方法，然后饿汉式一开始就开始创建，并提供get方法
     */
    private static UploadImageThreadPoolManager mInstance = new UploadImageThreadPoolManager();
    public static UploadImageThreadPoolManager getInstance() {
        return mInstance;
    }

    private int corePoolSize= 1;//核心线程池的数量，同时能够执行的线程数量
    private int maximumPoolSize;//最大线程池数量，表示当缓冲队列满的时候能继续容纳的等待任务的数量
    private long keepAliveTime = 1;//存活时间
    private TimeUnit unit = TimeUnit.HOURS;
    private ThreadPoolExecutor executor;
    private UploadImageThreadPoolManager() {
        /**
